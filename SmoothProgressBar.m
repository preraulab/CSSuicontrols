classdef SmoothProgressBar < CSSUIProgressBar
    ...
    methods
        ...
        function updateIteration(obj, iteration)
            ...
            if iteration < 1
                % Guard against `updateIteration(0)` and negative values
                % from caller bookkeeping bugs. The bar's "no progress"
                % state is what `start()` and `reset()` already produce;
                % silently no-op here rather than emitting a phantom
                % iteration that JS would render as 0% width with stale
                % timing state.
                return;
            end
            ...
            obj.pushCmd(upd);
            % drawnow flushes MATLAB's render queue. The 5 ms pause that
            % used to follow was guarding against the very narrow case
            % of an immediate setPrefix push clobbering this updateAnim
            % before JS read it. In the real run loop, updateIteration
            % is called once per (file, channel) work unit (seconds to
            % minutes apart) so back-to-back Data writes don't happen
            % and the pause was just dead wait — ~10 ms × every tick.
            drawnow
        ...
        function set.LabelPrefix(obj, val)
            obj.LabelPrefix = val;
            % Push the new prefix unconditionally — JS handles the
            % S=null case (drops the command) gracefully. Gating on
            % Running_ silently dropped prefix updates after auto-
            % complete fired (Running_=false at that point), so any
            % per-channel suffix logic added after the last
            % updateIteration would never paint.
    ...
    methods (Access = protected)
        ...
        function html = buildHTML(obj)
            ...
            script = [ ...
                ...
                '}' ...
                ... % Iteration-anchored pct with a sigmoid ease between ticks.
                ... % Each (current -> current+1) interval is a logistic
                ... % S-curve: 0 at sinceBase=0, ~0.5 at sinceBase=avgIterMs/2,
                ... % asymptoting to 1 as sinceBase -> infinity. Steepness
                ... % k=6 gives ~0.95 at avgIterMs (so a "well-behaved" tick
                ... % visibly almost completes its slot) while still being
                ... % asymptotically bound so the bar can NEVER overshoot
                ... % the next tick's mark, even when an iteration runs
                ... % many times longer than the EMA predicted. Combined
                ... % with the iteration ceiling at (current+1)/N, the bar
                ... % cannot read 100% until current == N.
                'function step(ts){' ...
                    'if(!S||!S.running){raf=null;return;}' ...
                    'if(S.avgIterMs>0){' ...
                        'var sinceBase=ts-S.baseTs;' ...
                        'var x=6*(sinceBase/S.avgIterMs-0.5);' ...
                        'var s=1/(1+Math.exp(-x));' ...
                        'var s0=1/(1+Math.exp(3));' ...
                        'var frac=Math.max(0,Math.min(1,(s-s0)/(1-s0)));' ...
                        'var pct=100*(S.current+frac)/S.N;' ...
                        ...
                        'var remIters=Math.max(0,S.N-S.current);' ...
                        'var trem=Math.max(0,remIters*S.avgIterMs-sinceBase);' ...
                        'setLabel(fmtLabel(pct,mmss(trem)));' ...
                ... % Pre-first-iteration "starting up" state. Bar width
                ... % follows the actual current/N (so it reads 0% when
                ... % current=0, matching the "0/N 0.0%" label) and the
                ... % pulse is communicated entirely via opacity. The
                ... % old version forced width >= 100/N, which painted
                ... % a 0.83% sliver while the label said "0.0%" -- a
                ... % small but visible width/label disagreement.
                    '}else{' ...
                        'var dur=1100;' ...
                        'setOpacity(0.55+0.45*Math.cos(' ...
                            '2*Math.PI*((ts-S.pulseT0)%dur)/dur));' ...
                        'var pulsePct=100*S.current/Math.max(S.N,1);' ...
                        'setProg(pulsePct);' ...
                        'setLabel(fmtLabel(pulsePct,"--:--"));' ...
                    '}' ...
