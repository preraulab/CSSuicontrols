classdef (Abstract) CSSBase < handle
    ...
    %                 Scrollbars (logical, default: false) — when true,
    %                            the widget renders thin styled scrollbars
    %                            for any overflow:auto region inside it
    %                            (Table body, ListBox track, Tree wrap).
    %                            When false (default) scrollbars are
    %                            hidden globally; wheel/keyboard scrolling
    %                            still works but there's no visible track.
    %
    %   Outputs:
    ...
    properties (Access = protected)
        ...
        TempDir_    = ''
        Scrollbars_ = false          % whether to render visible scrollbars
    end
    ...
    methods (Access = private)
        ...
        function html = assembleHTML(obj, componentHTML)
            ...
            % Scrollbars_=false  (default): hide native scrollbars across
            % every overflow region in this widget. Wheel/keyboard scrolling
            % still works.
            % Scrollbars_=true: render thin styled scrollbars (8px, semi-
            % transparent thumb) so users can see how much content overflows
            % and drag to scroll. Subclasses with overflow:auto regions
            % (Table, ListBox, Tree) get this for free.
            if obj.Scrollbars_
                scrollbarCSS = [ ...
                ...
                htmlScrollbarCSS = '';   % let scrollbar-width default to auto
            else
                scrollbarCSS = [ ...
                ...
                htmlScrollbarCSS = ['scrollbar-width:none;-ms-overflow-style:none;'];
            end
            infraCSS = [ ...
