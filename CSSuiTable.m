classdef CSSuiTable < CSSBase
    ...
    %       'RowClickMode'        : char - 'replace' (default) | 'toggle'.
    %                               'toggle' makes a plain click toggle the
    %                               clicked row's selected state. Modifier-key
    %                               (Ctrl/Shift) behavior is unchanged.
    %       'ColumnResizable'     : logical - drag the right edge of any
    %                               header to resize that column (default:
    %                               false). New widths sync back into
    %                               ColumnWidth so they survive re-renders.
    %       'ColumnDividers'      : logical - subtle vertical dividers
    %                               between columns (default: false).
    %       'SelectionOrder'      : char - 'visual' (default) | 'click'.
    %                               'visual' returns Selection in DOM/row
    %                               order. 'click' tracks the order rows
    %                               were selected, so callers can tell
    %                               which row was picked first.
    ...
    properties (Access = public)
        ...
        RowStriping         = true
        RowClickMode        = 'replace'  % 'replace' (clear+select) | 'toggle'
        ColumnResizable     = false   % drag column borders to resize widths
        ColumnDividers      = false   % subtle vertical dividers between columns
        SelectionOrder      = 'visual' % 'visual' (DOM order) | 'click' (click order)
    end
    ...
    methods
        function obj = CSSuiTable(parent, options)
            arguments
                ...
                options.RowClickMode    (1,:) char    = 'replace'
                options.ColumnResizable (1,1) logical = false
                options.ColumnDividers  (1,1) logical = false
                options.SelectionOrder  (1,:) char    = 'visual'
            ...
            obj.RowClickMode        = options.RowClickMode;
            obj.ColumnResizable     = options.ColumnResizable;
            obj.ColumnDividers      = options.ColumnDividers;
            obj.SelectionOrder      = options.SelectionOrder;
        ...
        function html = buildHTML(obj)
            ...
            for c = 1:nCols
                ...
                if obj.ColumnResizable
                    resizerHTML = sprintf('<span class="col-resizer" data-col="%d"></span>', c);
                end
                hdrHTML = [hdrHTML sprintf('<th data-col="%d"%s>%s%s</th>', ...
                    c, wStyle, label, resizerHTML)]; %#ok<AGROW>
            end
            ...
            css = [ ...
                ...
                'background:rgba(100,140,200,0.55);' ...
                '}' ...
                'td{' ...
                'padding:5px 10px;' ...
                ...
                '}' ...
                '.cssui-dividers th:last-child,.cssui-dividers td:last-child{' ...
                'border-right:none;' ...
                '}' ...
            ...
            clickModeJS = ['var _rowClickMode="' obj.RowClickMode '";'];
            selOrderJS = ['var _selOrderMode="' obj.SelectionOrder '";var _selOrder=[];'];
            if obj.ColumnResizable, crStr = 'true'; else, crStr = 'false'; end
            colResizableJS = ['var _colResizable=' crStr ';'];
            ...
            compJS = [ ...
                ...
                'cur.push(parseInt(r.getAttribute("data-row")));});' ...
                'if(_selOrderMode==="click"){' ...
                '_selOrder=_selOrder.filter(function(r){return cur.indexOf(r)>=0;});' ...
                'cur.forEach(function(r){if(_selOrder.indexOf(r)<0)_selOrder.push(r);});' ...
                'sel=_selOrder.slice();' ...
                '}else{sel=cur;}' ...
                ...
                '}else if(_rowClickMode==="toggle"){' ...
                'row.classList.toggle("selected");' ...
                '_rowAnchor=row.classList.contains("selected")?row:null;' ...
                '}else{' ...
                ...
                '});' ...
                '}' ...
                'function _attachColumnResizers(){' ...
                'if(!_colResizable)return;' ...
                'document.querySelectorAll(".col-resizer").forEach(function(handle){' ...
                'handle.addEventListener("mousedown",function(e){' ...
                'e.preventDefault();e.stopPropagation();' ...
                'var col=parseInt(handle.getAttribute("data-col"));' ...
                'var th=handle.parentElement;' ...
                'var startX=e.pageX;var startW=th.offsetWidth;' ...
                ...
                'window.sendEvent({event:"columnresize",col:col,width:th.offsetWidth});' ...
                '}' ...
            ...
            rootCls = 'cssui-table';
            if obj.ColumnDividers, rootCls = [rootCls ' cssui-dividers']; end

            html = [ ...
                '<!DOCTYPE html><html><head><style>' css '</style></head><body>' ...
                '<div id="css-root" class="' rootCls '">' ...
            ...
            switch data.event
                ...
                case 'columnresize'
                    if obj.Enabled_
                        c = double(data.col);
                        w = double(data.width);
                        if c >= 1 && isfinite(w) && w > 0
                            if numel(obj.ColumnWidth_) < c
                                obj.ColumnWidth_(end+1:c) = NaN;
                            end
                            obj.ColumnWidth_(c) = w;
                        end
                    end
                case 'edit'
                    if obj.Enabled_
